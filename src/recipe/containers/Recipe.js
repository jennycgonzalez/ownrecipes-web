...
class Recipe extends React.Component {
  ...
  render() {
    const { lists, recipes, match, status, user } = this.props;
    const { recipeActions, recipeItemActions } = this.props;
    const { showItemModal } = this.state;
    const data = recipes.find(t => t.slug === match.params.recipe);
    if (data) {
      const showEditLink = (user !== null && user.id === data.author);
