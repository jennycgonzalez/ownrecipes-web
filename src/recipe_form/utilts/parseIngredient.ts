...
export default (line: string): IngredientInput => {
  ...
  if (tags.length === 1) {
    ...
    if (Number.isNaN(parseFloat(tags[0][0]))) {
      return { title: line };
    } else {
      const { amount, measurement } = numberSplit(tags[0]);
      if (measurement) {
    ...
    if (Number.isNaN(parseFloat(tags[0][0]))) {
      ...
      // then the whole thing is the title.
      return { title: line };
    } else if (Number.isNaN(parseFloat(tags[1]))) {
      ...
      const quantity = tags.splice(0,2);
