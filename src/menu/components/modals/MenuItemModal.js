...
class MenuItemModal extends BaseModal {
  ...
  componentWillReceiveProps(nextProps) {
    const { event, startDate } = nextProps;
  ...
  onStateDateChange = (name, value) => {
    ...
    const newState = {};
    newState[name] = momentValue;
    this.setState(newState);
  };

  render() {
    const { id, show, onHide, fetchRecipeList, intl } = this.props;
    const { recipe, title, placeholder, start_date, complete } = this.state;
    ...
    return (
      <Modal show={show} onHide={onHide} className='rbc-calendar-modal'>
          ...
          {id !== 0
              ? (
                <div
                    ...
                    onClick={this.remove.bind(this, intl.formatMessage(messages.confirmDelete))}>
        ...
        <Modal.Footer>
          ...
          <button type='button' className='btn btn-primary' onClick={onHide}>Cancel</button>
        </Modal.Footer>
      </Modal>
    );
  }
